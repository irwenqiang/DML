...
void OPT_ALGO::split_line(const std::string& split_tag, std::vector<std::string>& jobVec){
	jobVec.clear();
	size_t start = 0, end = 0;
	while((end = line.find_first_of(split_tag, start)) != std::string::npos){
		if(end > start){
			index_str = line.substr(start, end - start);
			jobVec.push_back(index_str);
		}
		start = end + 1;
	}
	if(start < line.size()){
...
void OPT_ALGO::get_feature_struct(const std::vector<std::string>& feature_index){
	for(size_t i = 1; i < feature_index.size(); i++){
		size_t start = 0, end = 0;
...
void OPT_ALGO::load_data(const std::string& data_file, const std::string& split_tag) {
...
//----------------------------owlqn--------------------------------------------
float OPT_ALGO::sigmoid(float x) const {
	return 1 / (1 + exp( -x ));
}

float OPT_ALGO::loss_function_value(float *para_w) const {
	float f = 0.0;
	for(size_t i = 0; i < fea_matrix.size(); i++){
		float x = 0.0;
		for(size_t j = 0; j < fea_matrix[i].size(); j++){
			int id = fea_matrix[i][j].idx;
			float val = fea_matrix[i][j].val;
			x += *(para_w + id) * val;   // maybe add bias later
		}
		float l = label[i] * log(1/sigmoid(-x)) + (1 - label[i]) * log(1/sigmoid(x));
		f += l;
	}
	return f;
}

void OPT_ALGO::loss_function_gradient(float *para_w, float *para_g) const {
	// float f = 0.0;
	for(size_t i = 0; i < fea_matrix.size(); i++){
...
void OPT_ALGO::loss_function_subgradient(float * local_g, float *local_sub_g) const {
	if(c == 0.0) {
		for(int j = 0; j < fea_dim; j++){
			*(local_sub_g + j) = - *(local_g + j);
		}
	} else {
		for(int j = 0; j < fea_dim; j++){
			if(*(w + j) > 0){
...
void OPT_ALGO::two_loop(float *local_sub_g, 
						float **s_list, 
						float **y_list, 
						float *ro_list, 
						float *p) {
...
void OPT_ALGO::fix_dir(float *w, float *next_w) const {
	for(int j = 0; j < fea_dim; j++){
...
void OPT_ALGO::line_search(float *param_g){
	// DEBUG(zhangyule)
	std::cout << "line_search start! " << std::endl;
	float alpha = 1.0;
	float beta = 1e-4;
	float backoff = 0.5;
	...
	std::cout << "linear search Job done! " << std::endl;
}
